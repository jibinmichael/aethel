...
function AppContent() {
  ...
  // Simplified nodes change handler - removed complex change detection
  ...
  }, [onNodesChange, popover.isOpen, popover.sourceNodeId, recalculatePopoverPosition])

  // Simplified edges change handler - removed auto-save
  ...
  }, [popover.isOpen, popover.sourceNodeId, recalculatePopoverPosition])
  ...
  }, [nodes, edges, setNodes, setEdges, generateRefId])
  ...
  const handleNodeTypeSelect = useCallback(async (nodeType) => {
    ...
    const currentUser = guestManager.getCurrentUser()
    const newNode = {
      id: newNodeId,
      type: nodeType.id,
      position: { 
  ...
  }, [nodes, edges, popover.sourceNodeId, setNodes, setEdges, handlePopoverClose, generateRefId])
  ...
  const handleDeleteNode = useCallback((nodeId) => {
    setNodes((prevNodes) => {
      ...
      if (nodeToDelete && nodeToDelete.data.createdBy !== currentUser?.id) {
        console.log('❌ Cannot delete node: not owned by current user')
        return prevNodes
      }
